...
#define vec_set(v, value, idx, len)\
  for (size_t i__=(idx); i__<=(len); i__++)\
    (v)->data[i__] = (value);

#define vec_fill(v, value, idx, len)\
  do {\
    vec_insert_gap_(vec_repack_(v), (idx), (len));\
    vec_set(v, value, idx, len);\
  } while (0)
...
#define vec_append_str(dst, src)\
  vec_insert_str(dst, src, (dst)->len)
