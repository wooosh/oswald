...
void mark_move_rel(struct Mark *m, ssize_t x, ssize_t y) {
  m->y = clamp(0, m->buffer->lines.length, y);
  m->x = clamp(0, mark_line(m)->contents.length, x);
}
...
void mark_delete(struct Mark *a, struct Mark *b) {
  ...
  if (start->y == end->y) {
    vec_splice(&mark_line(start)->contents, start->x, end->x);
    vec_splice(&mark_line(start)->highlight, start->x, end->x);
    return;
  }
  ...
  vec_truncate(&mark_line(start)->contents, start->x);
  vec_truncate(&mark_line(start)->highlight, start->x);
  
  vec_splice(&mark_line(end)->contents, 0, end->x);
  vec_splice(&mark_line(end)->highlight, 0, end->x);
  ...
  for (size_t i=start->y + 1; i<end->y; i++) {
    ...
    vec_deinit(&lines->data[i].contents);
    vec_deinit(&lines->data[i].highlight);
  }
...
void mark_insert(struct Mark *m, vec_const_char_t str, bool keep_pos) {
  vec_insert_vec(&mark_line(m)->contents, m->x, &str);
