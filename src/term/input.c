...
void input_buffer_refill(struct InputBuffer *inbuf) {
  ...
  if (ready > 0) {
    if (pfd.revents & POLLIN) {
      // refill buffer
      size_t result = read(STDIN_FILENO, inbuf->seq + inbuf->available,
                           MAX_ESC_LEN - inbuf->available);
...
struct Key term_read_key() {
  ...
  if (starts_with(inbuf.seq, "\x1b[")) {
    for (int i = 0; escape_rules[i].key != KeyNone; i++) {
      ...
      if (starts_with(inbuf.seq, rule.pattern)) {
        ...
        if (!starts_with(inbuf.seq, "\x1b[1;"))
          continue;
        if (rule.pattern[2] != inbuf.seq[5])
          continue;
  ...
  struct Key k = {inbuf.seq[0], .shift = isupper(inbuf.seq[0]),
